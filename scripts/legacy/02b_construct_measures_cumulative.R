# code/02b_construct_measures_cumulative.R
...
# It leaves the original three-schedule EPE pipeline unchanged and creates a new
...
source("functions/make_max_cal_by_tooth.R")
source("functions/calc_severity_extent.R")
...
demo_ghb <- readRDS("data/derived/demographics_hba1c.rds")
...
# ============================================================
# 4. Construct conventional site-level severity and extent measures
# ============================================================

sev_ext_site <- purrr::imap_dfr(ohx_list, function(df, nm) {
  out <- calc_severity_extent_site(df)
  out |>
    dplyr::mutate(source_cycle = cycle_from_filename(nm))
}) |>
...
sev_ext_site <- sev_ext_site |>
  dplyr::semi_join(
    CAL_wide |> dplyr::select(SEQN) |> dplyr::distinct(),
    by = "SEQN"
  )

message("Observed sites summary:")
print(summary(sev_ext_site$n_sites_observed))
...
readr::write_csv(
  ...
  "data/derived/incident_periodontal_attribution_lookup.csv"
)
...
readr::write_csv(
  ...
  "data/derived/cumulative_periodontal_attribution_lookup.csv"
)
...
# Points show survey-weighted mean number of missing teeth by single year 
# of age, with standard errors estimated using the NHANES complex survey 
# design. The solid line shows a spline-smoothed, non-decreasing estimate of 
# the age-specific mean number of missing teeth, used to approximate accumulated 
# tooth loss in the cumulative attribution model.
...
# ============================================================
# 7. Cumulative EPE calculation
# ============================================================
# Retained teeth contribute observed tooth-level max CAL.
# Missing teeth contribute:
...
EPE_cumulative <- calc_EPE_cumulative(
  ...
  dplyr::mutate(Schedule = "Cumulative attribution model")
...
analytic_cumulative <- analytic_cumulative |>
  dplyr::left_join(sev_ext_site, by = "SEQN") |>
  dplyr::rename(
    severity = mean_CAL_site,
    extent_ge_3mm = pct_sites_ge_3mm,
    extent_ge_5mm = pct_sites_ge_5mm,
    extent_ge_6mm = pct_sites_ge_6mm
  )

readr::write_csv(
  analytic_cumulative,
  "data/derived/analytic_dataset_cumulative.csv"
)

saveRDS(
  analytic_cumulative,
  "data/derived/analytic_dataset_cumulative.rds"
)

message("Saved: data/derived/analytic_dataset_cumulative.csv")
message("Saved: data/derived/analytic_dataset_cumulative.rds")
